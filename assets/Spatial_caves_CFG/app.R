...
server <- function(input, output, session) {
  ...
  output$mymap <- renderLeaflet({
        leaflet() %>% addCircleMarkers(data = caves,~Longitude, ~Latitude,color=pal_cave(caves$Cave_Type),radius= 1.5, popup = ~as.character(Cave_ID), label = ~as.character(Cave_Name),group = "Caves") %>% addPolygons(data=katafygia_agrias_zwhs_simple ,color = "orange", weight = 0.3, smoothFactor = 0.5, opacity = 0.4, fillOpacity = 0.4,fillColor = "orange",highlightOptions = highlightOptions(color = "white", weight = 2, bringToFront = TRUE),group = "Wildlife refugee") %>% addPolygons(data=natura2000_new_shapefile_v30 ,color = "springgreen2", weight = 0.3, smoothFactor = 0.5,opacity = 0.6, fillOpacity = 0.5,fillColor = ~pal_natura(natura2000_new_shapefile_v30@data$SITETYPE),highlightOptions=highlightOptions(color = "white", weight = 2,bringToFront = TRUE),group = "Natura 2000") %>% addProviderTiles("Stamen.Terrain", group = "Terrain") %>%  addProviderTiles("Esri.WorldImagery", group = "World Imagery") %>% addProviderTiles("Stamen.TonerLite", group = "Toner") %>% addLegend("bottomright", pal = pal_natura, values = natura2000_new_shapefile_v30@data$SITETYPE, title = "Natura2000", labFormat = labelFormat(prefix = ""), opacity = 1) %>% addLayersControl( baseGroups = c("Toner","Terrain","World Imagery"), overlayGroups = c( "Caves","Wildlife refugee","Natura 2000"), options = layersControlOptions(collapsed = FALSE))
  })
}
